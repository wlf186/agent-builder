...
它**不是生产就绪产品**。当前已有通用 Agent Capsule create/upgrade/delete，但仍只有
固定模型和 `builtin/echo`；没有 Shell、文件工具、Skill、MCP、RAG、子智能体或 TLS。原型适合在
...
Authenticated Web Gateway / trusted Control Plane
    ...
    └── bounded stdio IPC
        ...
        Landlock + seccomp + rlimits
             ...
             ▼
        HarnessKernel
        context-plan reference → model → echo → model → echo → model → terminal
```
...
当前 Echo 演示固定最多 2 次顺序调用：不可变 TurnRuntimeSnapshot 固化最多 4 次模型调用、
