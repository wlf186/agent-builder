...
Authenticated Web Gateway / trusted Control Plane
    ├── session CRUD → ConversationStore / Agent state.sqlite (WAL)
    ├── CommandBus → RunService → canonical sequencer → durable journal
...
Web 页面可以新建、选择恢复和删除会话，并在同一会话连续对话；回答、工具阶段和
事件时间线仍增量显示。点击任一事件可查看未经界面截断的 canonical event envelope；
消息通过纯文本渲染，不会作为 HTML 执行。
...
重连。该能力只适用于当前 Gateway 内存 retention；活跃流、ephemeral delta 和事件
cursor 不跨 Gateway 进程恢复。
