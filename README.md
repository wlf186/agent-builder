...
composer 的上下文主指标来自服务端 next-turn projection，展示当前 committed 固定上下文的
“占用约 X / 模型窗口”和剩余百分比，并在次级信息回答下一条消息安全可写量与自动整理前
余量；不可用时明确说明原因而不猜数字。上一轮 Provider 实际用量只作弱化的计算量参考。
窄屏下导航和
...
重命名、分支、分页恢复和删除会话，并在同一会话连续对话。Run 在切换会话后继续后台
接收 SSE；每个会话独立保留运行标记、草稿、模型和整理选项，切回后恢复实时视图。中央按
durable Turn 展示用户消息、成功回答和带安全错误码/阶段/耗时的非成功终态；可重试失败只
把原消息填回输入框，必须由用户再次发送才能创建新 Turn。点击 Turn 会打开所选 Run 的详情，并把 canonical events 按
...
prompt 从未因此暴露。用户消息按纯文本渲染；完成的 assistant 消息使用受限 DOM Markdown，
原始 HTML 保持文本，链接只允许无凭据的 http/https 且不加载远程图片或其它资源。
会话右侧操作使用浏览器 top-layer Popover，并按当前 visual viewport 在触发器上方或下方
展开后夹紧到视口；重命名、从当前结尾分支和删除不会再被会话列表的滚动边界裁切。
终态对账时 durable assistant 是唯一正文事实；同一 Run 已有 canonical assistant 后，浏览器
不会再次把内存中的 live assistant buffer 投影成另一张 Turn 卡。
...
当前 Ollama 生成链路固定使用流式 `POST /api/chat`；运行时不调用 `/api/generate`，也不允许
浏览器或 Worker 选择 Provider 路径。普通无 Tool 回答固定使用
`temperature=1.0/top_p=0.95/top_k=20/presence_penalty=1.5/seed=0`；带 Tool schema 的选择阶段
固定使用 `temperature=0/seed=0`。这些参数属于受信 generation policy，浏览器、Worker 和
单次请求均不能覆盖。
...
ContextPlan 同时记录模型 digest、原生/运行窗口、输出预留、ToolSpec digest、会话历史投影
和四个不可混用的计数域：`AdmissionUpperBound` 只用于硬安全准入，
`ProviderObservedUsage` 只描述绑定 exact request digest 的实际 input/output，带误差的
`SoftContextEstimate` 用于自动压缩，服务端 `NextTurnProjection` 用于用户发送下一条消息前
的容量预览。未来切换模型时，所有计数按新 profile/renderer/ToolSet 重建，不把 `28672` 或
某个模型名称写进会话数据，也不把 UTF-8 byte 上界冒充真实 token。

Web 的上下文主指标用 `NextTurnProjection.fixed_context_tokens` 与同一投影的
`operational_context_tokens` 展示当前 committed state 的占用和窗口剩余百分比；
`safe_user_tokens`、`compact_before_user_tokens`、误差与上一轮 Provider input/output 是次级
信息。样本不足、active Run、scope 漂移或容量状态无法可靠投影时，界面显示
unavailable/stale 原因，不用 byte 上界或旧 Provider usage 猜测替代值。
服务端首先按下一条消息可能启用的 EffectiveToolSet 生成 `conservative_tools` 主投影；Web
只要该投影可用就优先展示。若其 ToolSet scope 尚无校准，但严格独立的空 ToolSet scope 已有
完整观测，响应可另带 `chat_only_projection`，界面明确标成“纯对话基线”；它的 ratio、误差
和余量不会转移到工具场景，下一条消息实际启用工具时仍在 Turn admission 前重新编译和准入。

完成 Turn 以 `CompletedTurnContext` 保存有界、顺序化的 user、assistant Tool call、低权限
Tool result receipt 和 final assistant。自动或手动压缩从最旧端移动完整 bundle，使用经过
Provider observation 校准的 soft estimate 和明确误差；每层投影后仍独立执行硬 admission。
校准样本来自 Agent SQLite 中最近 16 条完整 Provider usage，且必须同时匹配 model profile、
renderer、ToolSet 与 compression policy；因此 Gateway 重启后的 preview、Run admission 和
自动压缩使用同一依据，零值或 incomplete response 不会制造虚假精度。
重复 guard 主动关闭 Provider stream 后无法取得可信终帧 usage，因此对应 soft preview 会在
新的同 scope 完整 observation 到来前明确显示 unavailable；这不会阻断下一轮提交。下一轮仍
从已提交的 `CompletedTurnContext`（含截断 marker）重新渲染，并以 exact request 的
`AdmissionUpperBound` 做硬准入，不会把中止调用的零计数当成窗口占用。
合法 Turn 在首次模型调用前还会为最大受准 final、Tool receipt 和最小下一条消息预留
continuation headroom；SQLite canonical transcript 从不因投影、摘要或模型切换而删除。

`semantic-summary-v1` 永久禁止新建，只保留严格旧 codec。当前默认启用已经重新资格的
`semantic-summary-v2`；operator 可在启动前设置 `HARNESS_V2_SEMANTIC_SUMMARY_V2=0` 紧急
回退到纯 deterministic collapse。v2 的规则位于独立可信 system role，source transcript 和
生成摘要都只以低权限 conversation data 进入主模型，ToolSet 固定为空。相同 source 跨 Turn/
Gateway 重启零 HTTP 复用，source 扩展使用 parent snapshot + 新 completed bundle 增量更新；
失败、超限、timeout 或 circuit open 都安全回退，不修改 canonical transcript。完整迁移、
回滚和资格证据见
[Context reliability remediation plan](docs/plans/context-reliability-remediation.md)。

`utf8-bytes-upper-bound-v1` admission fallback 在没有模型专用 tokenizer 时把每个
UTF-8 byte 按一个 token 计安全上界，并计入实际 section labels/roles、Provider Tool schema 和
...
轮次都会对完整投影 transcript + 当前 Provider Tool schema 重做硬预算 admission；移出所有可
省略的完整历史 Turn bundle 后仍超限时，以 `model_context_limit` fail closed，不让 provider
静默截断 prompt。
...
受信 Agent 指令要求创作和自包含问答默认直接生成；算术表达式使用常规运算优先级并在回答
前复核，不为这种自包含计算调用外部 Tool。
...
| 操作 | 路径 |
| --- | --- |
| Agent create/list/get/rename | `POST|GET /api/agents`、`GET|PATCH /api/agents/<agent-id>` |
...
| 分页读取、重命名、删除会话 | `GET|PATCH|DELETE /api/agents/<agent-id>/sessions/<conversation-id>`；读取支持 `limit=1..64&before=<opaque-cursor>` |
| 从 completed head 续接/分支 | `POST /api/agents/<agent-id>/sessions/<conversation-id>/continue`，`mode=continue|branch` |
| 查看/取消本轮上下文准备 | `GET /api/agents/<agent-id>/sessions/<conversation-id>/preparation`、`POST .../preparation/cancel` |
| 查看下一轮安全余量 | `GET /api/agents/<agent-id>/sessions/<conversation-id>/context-preview` |
...
preparation `GET` 在 idle 时返回 `operation_id=null`；preparing/cancelling 期间返回本次操作
稳定且非空的 ID，并在 admission 交接为 active Run 时保持不变。取消 POST 的 JSON 只能是
`{"operation_id":"<GET 返回的 exact ID>"}`。`202` 仅表示 control request 已确认，不能当作
准备或 Run 已经停止；客户端继续 GET，直到 state 为 idle 才能展示“已安全取消”。若该 ID
已被新一轮准备替代，响应为 `stale`，旧请求不得误取消新操作；handoff 后同一个 ID 会把取消
定向到对应 Run。

Conversation 是持久容器，Turn 是一次用户提交及其终态，Run 是执行该 Turn 的独立
Worker/模型会话。一个 Conversation 同时最多有一个 active Run；活跃时再次发送、分支或
删除会返回冲突，先取消或等待终态；其它 Conversation 可以同时浏览或独立运行。详情默认
只返回最近 32 个 Turn；客户端只能回传响应中的 `page.next_before_cursor` 向前加载，不能
构造 Turn position。`turn-page-v2` cursor 经认证并绑定 Agent、Conversation、revision 和
页边界；篡改、跨会话复用、rename 后旧 cursor 或 Gateway 重启后的旧 cursor 都以
`invalid_session_cursor` 拒绝。服务端每页只从 SQLite 读取该页的有界 Turn 正文，不复制、
改写或预读完整 canonical transcript。
成功终态会原子保存完整 completed Turn bundle；failed、
...
`Tool/恢复 → Harness`、Harness 内部状态和 replay control。每次逻辑模型调用固定增加
一个 `model.request.started` 和一个 `model.response.finished` durable 边界；每次实际首帧
HTTP attempt 各增加一对不含 endpoint/prompt/正文的 `model.transport.attempt`
started/finished 诊断，因此实际尝试数为 `N` 时总数为 `2 + 2*N`，且不按 frame、token 或
timer tick 写入。逻辑轮次、恢复 attempt、provider 调用序号和 transport attempt 分开显示。精确 context/media overflow 在任何 provider frame
...
- 同一受信 model/profile 的实际 HTTP attempt 连续两次零首帧失败后，Broker 开启 30 秒
  有界健康短路并快速返回可重试 `model_temporarily_unhealthy`；成功首帧立即复位，状态表
  最多 16 项且不持久化 endpoint 或跨 Gateway 重启继承。
- 默认 qwen 的单次输出预算为 4096 tokens；这为长回答和模型内部推理留出余量，同时仍受
  12 KiB 可见正文、1 MiB/4096 Provider 原始流上限、120 秒模型调用和 240 秒 Run deadline
  的多重上限。
- 普通无 Tool 回答在最近 2048 个 Unicode code point 内出现精确 suffix cycle 时，guard
  只接受 32..512-byte cycle、至少 3 份且累计至少 512 bytes 的证据，并按每 64 个新接收
  bytes 检查一次。命中后立即关闭 `/api/chat`，保留已发送正文、删除尚未发送的重复尾部，
  并追加一次 `[回答因重复死循环被截断；后续忽略重复尾部。]`。该 Turn 以
  `run.completed.reason=repetition_truncated` 完成，Provider usage 标为 incomplete；用户明确
  要求逐字重复大段内容时也可能触发，这是为有界资源选择的已知取舍。
- 普通文本回答达到 12 KiB commit ceiling 时，Broker 只提交 UTF-8 完整边界内的有界前缀和
  一次受信截断标记，并继续有界排空 `/api/chat` 流直至取得、校验最终 usage；该 Turn 以
  `run.completed.reason=max_output` 完成。达到上限时若同时出现 Tool call、没有可见正文，
  或终帧/usage/协议不合法，仍 fail closed，不把不可用输出包装成成功回答。
- 1 MiB/4096-frame Provider 原始流会被合并成最多 128 个 content IPC frame；受信 Broker
