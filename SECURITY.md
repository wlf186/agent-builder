...
超过动态 80% 阈值时，`completed-turn-collapse-v2` 只把最旧的完整 completed Turn pair
替换成内容无关的 receipt marker，至少保留最近一个完整 pair，并在 collapse 后重新执行
admission；durable transcript 不改变，所有可折叠旧 pair 耗尽后仍超出硬预算则 fail
...
语义摘要是 deterministic collapse 之后的可选层。其内部模型调用通过正常受信 Broker、
空 ToolSet、无 workspace/Capability 参数运行；输入 48 KiB、输出 8 KiB、15 秒、单次尝试，
五类数组各 16 项/384 bytes。摘要内容以 `conversation` trust 和明确 data-only guard 渲染，
源中的 prompt injection 不能获得 platform authority。失败立即回滚到已验证 collapse，三次
连续失败熔断 60 秒；operator 可在构造 Broker 时完全关闭。摘要不是事实证明，canonical
transcript 不被修改，最近完整 Turn 和 Tool identities 不被摘要替换。
...
资格缺失、能力漂移或未知 model ID 一律 fail closed。
...
重新验证，服务端不保存第二份偏好或压缩状态。所有 UI 内容用 `textContent`，diff、错误和
命令结果不能成为 HTML。
