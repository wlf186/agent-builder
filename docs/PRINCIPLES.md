...
可重建依赖也必须属于 Agent：固定研究环境保存在该 Agent 的 `dependencies/` 根并跨会话
复用，不能成为全局 site-package 或共享可写 venv。安装只接受受信固定 binary-wheel 清单；
删除环境只影响该 Agent，删除 Agent 时 dependency metadata、venv 和 staging 必须一并为零。

`0.2.0` 已贯通通用 Capsule create/upgrade/delete、generation promotion、按 Agent runtime
drain 和残留检查；正式系统 Agent 为控制面稳定性不可升级、不可删除，普通 Agent 可从 Web
管理抽屉完整管理。GNU/Linux x86_64 的 bounded
故障/残留/SSD workload 已关闭首发 gate；`aarch64`、长期 soak、SMART、互联网和多租户仍
不在支持范围，不能扩大 P8 结论。
