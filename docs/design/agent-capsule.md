...
`provisioning → active → upgrading|deleting`；系统 Agent ID
`00000000-0000-4000-8000-000000000001` 是不可升级、不可删除的正式默认入口。manifest schema 2 固化
...
不是多租户授权边界。系统 Agent 为保持控制面稳定而拒绝 upgrade/delete；普通 Agent 可在
Web 管理抽屉创建、切换、升级和删除，切换后所有 Command/Conversation/Run 请求都显式绑定
所选 Agent。首发仅支持
