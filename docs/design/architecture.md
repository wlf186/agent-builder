...
的完整成功观察。模型切换会清空旧 preview 并按新 profile 重算。
...
- Agent 指令采用 direct-answer-first Tool policy：只有外部/workspace 状态或用户明确要求的
  操作才使用 Tool，创作与自包含问答直接生成。其余 ToolSet 与执行权限保持不变；无业务
  Echo 在 policy 层移除，Tool 阶段结束再由末尾 finalization marker 做结构化相位切换，避免
  仅靠提示词约束小模型。任何残余第三次 Tool 请求以 `model_tool_loop` 精确分类且绝不执行。
