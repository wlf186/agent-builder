...
composer 的主指标回答“下一条消息还可写多少”：上一轮成功 terminal 后，取第一次完整
ToolSet 请求的 `model.response.finished.input_tokens`，加最终回答调用的 `output_tokens`，作为
下轮固定上下文的 Provider 实测推导基线，再分别减去 `compact_at_tokens` 和
`input_budget_tokens`。第一次请求覆盖 system、完整持久历史、本轮 user 和正常 ToolSet；最终
回答是该 Turn 新增的唯一持久 assistant 内容。中间 Tool call/result、重复读取和临时收窄
ToolSet 只进入本轮累计计算量，不叠加成下一轮持久上下文。Provider 尚未收到下一条消息，
chat template、下一条正文和未来 ToolSet 变化无法预先精确 tokenize，因此 UI 必须显示“约”。
`run.started.context_plan.estimated_input_tokens` 是 UTF-8 字节 admission 上界，只留在安全控制面，
不作为用户可见的 tokenizer 占用。辅助用量逐个去重汇总本轮所有
`model.response.finished` 的实际 input/output，并以 terminal aggregate `usage` 校准；Ollama
只在每次模型调用结束时报告 usage，文本 delta 期间不做逐 token 猜测。snapshot-only 的
单调用 Run 可由 aggregate 恢复该基线；多调用 Run 缺少逐调用边界时停止推导，不用最后一次
无 Tool input 或累计 output 制造误导数字。
