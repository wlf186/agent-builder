...
```text
┌──────────────────────────── Browser ─────────────────────────────┐
│ login · Agent/session rail · Turn transcript · on-demand replay  │
└───────────────────────────────┬───────────────────────────────────┘
...
│ research.py  Agent-private dependency lifecycle + document executor     │
...
完成 Turn 的后续模型历史统一服从
[Completed Turn context contract](completed-turn-context.md)：无 Tool Turn 保存原生
user/final assistant；有 Tool Turn 还按 call 顺序保存 assistant tool use 与低权限、带 digest
的 Tool result receipt。只有 completed terminal transaction 一次性晋升 bundle；failed、
cancelled、interrupted 或 partial 内容永不进入后续历史。platform/Agent/workspace sections
每轮从受信来源重新编译，不持久化为 Conversation message。

容量数字统一服从 [Context count domains](context-count-domains.md)。硬准入 byte 上界、单次
Provider observed usage、带误差的 soft estimate 与 committed next-turn projection 是四种不可
混用的类型；profile/renderer/ToolSet/policy 任一变化都会使比较和缓存失效。

浏览器只组合两类已经存在的读模型：中央 Conversation/Turn transcript 来自 session API，
按需打开的右侧检查器中，所选 Run 的规范时序来自 live SSE 或经过完整验证的 durable
replay。左侧仅导航 Agent/Conversation，不拥有业务状态。四泳道
...
`seq/event_id/durability`，也不能写回 journal。Turn 卡片选择对应 Run 并打开检查器，事件
节点反向定位拥有它的 Turn；两者都不能改变 Conversation 或 Run 的权威状态。默认隐藏
检查器只改变浏览器投影的可见性，不能停止 SSE、遗漏 durable event 或另建回放状态。
消息流的 follow-latest 是纯内存 UI 状态：用户接近底部时，流式 block 通过单个待处理 UI
任务合并滚动，显式恢复则立即吸底；用户主动回看后暂停，绝不改变 SSE 或 transcript。
Conversation detail 默认只投影最新 32 个 Turn；服务端以 SQLite 中不可变的 Turn position
作为内部边界，只向客户端暴露绑定 Agent/Conversation/revision/边界的认证 opaque cursor。
每页使用有界 SQL 读取，前端 prepend 时保持视觉滚动锚点并按 message identity 去重。active
Run、admission 准备、草稿、模型和 compact 选择以 Agent ID + Conversation ID 为 key；准备态
通过 prompt-free 阶段 API 观察，并可用独立 control path 取消。idle 状态的
`operation_id=null`；preparing/cancelling 的 ID 在阶段变化及 preparation→active Run handoff
期间保持稳定。取消只能提交该 exact ID；`202` 是受理确认而非终态，浏览器继续 GET 到 idle
才报告收敛。旧 ID 命中后来操作时返回 `stale` 且不发信号，以 operation identity 阻断 ABA。
该路径不等待 QueryEngine 的提交锁，且与 active Run handoff 共用一个 cancellation signal。
切换会话只让 SSE 进入后台，
不会取消 Worker、把 delta 写入错误会话或让完成状态抢走当前 composer。内存 timeline cache
只保留 active Run 和当前已打开检查器所需的单个历史 Run；关闭检查器、切换历史 Run 或
pagehide 会立即释放不再需要的正文/事件缓存，避免随历史浏览无界增长。

会话首轮准入时由用户消息确定性生成有界标题，不额外调用模型；重命名只更新展示元数据。
分支/续接固定发生在 completed head，源 transcript 保持不变且只传递有界、低权限 completed
projection。failed/cancelled/interrupted Turn 的详情只含安全 code/stage/retryable/duration；重试
按钮仅恢复草稿，新 Turn 仍完整经过 CSRF、active-state、byte、context 和 model admission。
assistant 完成消息通过 DOM 白名单渲染有限 Markdown，原始 HTML 保持文本、外链只允许无凭据
http/https，不解析图片或其它远程资源。
composer 的主指标回答“下一条消息还可安全写多少”：浏览器只消费认证、`no-store` 的
`next-turn-projection-v1`。服务端从当前 committed Conversation revision、模型 profile、
generation、ToolSet、renderer、prompt sources 与 completed Turn bundles 重编无正文的下一轮
投影，再用同 scope 的完整 Provider observation 校准 soft estimate 和误差。响应分别给出固定
上下文、误差、压缩前余量、硬余量、单消息 byte cap、turn capacity、strategy/basis/version；
active Run、容量耗尽、incomplete usage 或任一 scope 漂移都返回明确 unavailable/stale reason，
不猜测数字。failed/cancelled/interrupted 不晋升历史；overflow recovery 使用最终 active plan
的完整成功观察。repetition-truncated Turn 虽是 completed，但其提前关闭的 Provider 调用仍是
incomplete；此时 soft preview 明确 unavailable，下一条消息本身仍从该 committed bundle
重编，并使用 exact request 的 `AdmissionUpperBound` 做硬准入。模型切换会清空旧 preview
并按新 profile 重算。

本轮所有 `model.response.finished` 的实际 input/output 仍按 event ID 去重汇总，并以 terminal
aggregate 校准，只作为弱化的 Provider 计算量指标；Tool 循环会重复计算共享上下文，因此不与
next-turn baseline 相加。Ollama 只在模型调用终帧报告 usage，文本 delta 期间不逐 token 猜测。
`run.started.context_plan.estimated_input_tokens` 仍是硬 admission 上界，不冒充 tokenizer 或
用户可见上下文占用。
同 scope 最近 16 条完整 Provider observation 由 SQLite 重新构建 soft calibration；preview、
admission 和 compaction 共享该 builder，Gateway 重启不会回退到另一份 Broker 内存判断。
...
2. `CommandBus` 把普通 start 或显式控制命令交给有界 `QueryEngineRegistry`；Registry 为该 Conversation
   ...
   context bundle 快照。Control Plane 只从当前 Capsule 精确的
...
7. Control Plane 只把本轮用户消息、`plan_id`/plan digest/toolset digest 引用和经过
   ...
   Worker 只能从运行时密封的 catalog 解析该子集，用同一 manifest 校验模型
   Tool call；`file/stat`/`file/read_text`/`file/glob`/
   `file/grep`、`file/edit`、`file/write`、`exec/run` 和已安装时的
   `document/extract_text` 只能发出绑定
...
8. Broker 为该 Run 独占一个 `OllamaRunSession`，从受信 ContextPlan 渲染 system、
   completed history bundle 的原生 user/assistant-tool/tool/final roles 和当前 user message，
   并从相同 ToolSpec 生成
   ...
   usage 上限和 240 秒 wall deadline。模型可顺序请求两次当前 EffectiveToolSet 中的能力；
   文件请求由 Control Plane 对本 Agent Capsule 执行；每次结果都按原 call ID 回流，第二次
   预算消费后 provider ToolSet 收窄为空。Broker 随后在最后一个 Tool result 后追加固定、
   不插值任何不可信结果的受信 finalization marker，再进行最终模型调用。Echo contract 只
   留在密封 catalog 中验证旧 journal/原型测试，不属于新 Run 的 release EffectiveToolSet。
   call/result 重复、乱序、并行、超预算或 snapshot 漂移都 fail closed。
   Durable replay 对同一 digest 和 `visible_tools` 做严格复核；research、Skill 和 extension
   是彼此独立的可选 Tool，因此校验器只接受完整 runtime catalog 的确定性可选子集，不能
   把一个已安装可选能力误判为 journal corruption，也不能接受 catalog 外的任意组合。
9. Ollama session 在完整 provider request 已通过动态 Token/字节 admission、但尚未打开
   ...
   收敛为 `incomplete`。每次逻辑调用固定增加两个 request/response 语义边界；其每个实际
   HTTP attempt 另增加两个 transport started/finished diagnostic。若实际尝试数为 `N`，durable
   事件总数为 `2 + 2*N`；usage ledger 仍只随前后两个语义边界事务转换，不逐 token、frame
   或 timer 写盘。任何硬崩溃都不会留下“usage 已完成但 response 尚不存在”的不可恢复窗口。
   Worker 看不到也不能伪造 usage、request digest 或边界事件，terminal
   aggregate 必须与 provider ledger 完全一致。
   普通无 Tool response 若命中受信 exact-suffix guard，Broker 会删除尚未发出的重复尾部、
   追加固定 marker 并立即关闭 HTTP response；它不等待终帧、不重试且立即释放模型 slot。
   Control Plane 将 response 原子闭合为 `repetition_truncated/0/0/usage_complete=false`，将
   Turn 以同名 reason completed，并把唯一 assistant 正文晋升为 completed bundle。Tool phase、
   无正文、自然终帧和先到达的显式取消不允许走该分支。
   Provider 仅在 `400|413`、严格 JSON `error` 字段和窄 context/media 模式同时匹配时才算
   overflow；认证、网络、状态 200 的部分流和格式错误不会触发恢复。若首个 attempt 尚未
   产生 provider frame，Control Plane 可把已预 admission 的 recovery projection
   ...
   `complete=false`。
   一个逻辑 request 内部的每个实际首帧网络尝试另以一对 `model.transport.attempt`
   started/finished 记录 attempt/max、elapsed、TTFB 与固定 outcome；它不含 endpoint、request、
   response 或错误原文，也不改变 request/response boundary。相同受信 model/profile 连续两个 HTTP attempt 零首帧失败后，
   Broker 以最多 16 项的内存健康表开启 30 秒 circuit；open 时不占 slot、不发 HTTP，成功
   首帧立即复位。partial、idle 和 cancel 不计入该 circuit，Gateway 重启清空健康状态。
...
| `agents.py` | 最多 100 个 Agent 的 registry、provisioning/active/renaming/upgrading/deleting 状态和恢复 | live Run/QueryEngine、模型连接 |
...
| `research.py` / `research_bundle.py` | 固定 binary-wheel 清单、Agent-scoped 原子环境生命周期、文档快照与 singleton sandbox parser | 请求可控 package、运行期网络/install、跨 Agent 环境、任意代码执行 |
...
| `sessions.py` | Conversation/Turn 事务、completed bundle/summary/preview source、continuation、Run journal metadata、terminal snapshot、permission/operation/provider ledger、capability audit、重启恢复 | 模型 prompt、Worker 生命周期、Web principal |
...
| `context.py` | 确定性 sections/provenance、completed bundle projection、continuation preflight、动态预算、summary data guard 与 plan digest、正文隐藏的 operator inspection | provider/network selection、第二份 canonical transcript |
| `completed_context.py` | completed Turn item/bundle schema、顺序/身份/digest/大小与 Provider history renderer | terminal 事务、event replay、canonical Tool 原文 |
| `context_counts.py` | admission/observed/soft/next-turn 四个版本化计数域、scope 与校准 | Provider 调用、UI 自行估算、跨 scope 比较 |
| `semantic_summary.py` / `semantic_summary_v2.py` | v1 兼容 decoder；v2 五类有界 summary、source/parent/profile/prompt/policy/renderer/request/usage binding 与低权限渲染 | 模型调用、Tool 或 canonical transcript mutation |
...
| `ollama.py` / `repetition.py` | 受信 Catalog/endpoint、模型资格、per-Run transcript、ContextPlan 渲染、空 Tool summary 子调用、协议上限与有界 exact-suffix guard | 浏览器可配置 provider/endpoint、语义相似检测、无界重复扫描 |
| `tools.py` | 版本化 `ToolSpec`、受控 schema/result/progress vocabulary、确定性 Tool-result provider projection、ToolCatalog/Policy/EffectiveToolSet、历史 Echo contract 与 brokered file contract | permission 决策、Shell/Skill/MCP 动态执行 |
...
Agent 的显示名和运行代是两个独立生命周期。认证 `PATCH /api/agents/<id>` 只把目标名称持久
化为 `renaming`，原子替换同 generation manifest 后回到 `active`；它不经过 RuntimeManager
drain，不改变 Worker interpreter、Conversation、workspace、Skill 或依赖环境。认证
`POST /api/agents/<id>/upgrade` 才设置 admission fence、构建并资格检查下一 generation、原子
promotion 并清理旧环境。Web 因而把 rename 作为普通操作，把 generation upgrade 标为仅供
基础环境、安全策略或 Agent 定义实质变化时使用的高级操作。
...
- 后续 ContextPlan 只读取 `completed` Turn 的完整 context bundle。失败、取消、重启
...
| context assembly | trusted immutable `ContextPlan`、provenance/digest/budget、completed Turn 原生 chat roles | completed bundle、continuation、adaptive Tool receipt、deterministic collapse、semantic-summary-v2 和认证 metadata inspection 已重新资格；v1 只解码 |
...
| Tool registry/dispatch | `ToolCatalog → Policy Resolver → EffectiveToolSet → Broker/Worker/Executor` | release policy 不含 Echo；descriptor-anchored stat/read/glob/grep、审批式原子 edit/write 和 singleton `runtime-compile` 已贯通；通用 Bash 未实现 |
...
| cost/context compaction | 动态模型画像、四计数域、每调用 provider ledger、completed bundle、durable preview | soft/hard 域分离、summary v2 持久复用和 recovery active-plan 已完成 [CTX-R01 资格](../plans/context-reliability-remediation.md) |
...
普通无 Tool response 使用受信 `tool-phase-generation-v2` 的
`temperature=1.0/top_p=0.95/top_k=20/presence_penalty=1.5/seed=0`；存在 Tool schema 时使用
`temperature=0/seed=0`。选择依据只来自冻结的 provider phase，参数及完整 manifest 进入
generation-options digest，不能由请求数据覆盖。

模型容量不是按模型名称写死。启动时 Broker 从 `/api/show` 读取架构专属原生
`context_length`，再应用受信运行上限与输出预留。当前 `qwen3.5:2b` 报告 `262144`；
运行窗口为 `min(262144, 32768) = 32768`，输出预留 `4096`，硬输入预算 `28672`。
压缩策略阈值按硬输入预算动态计算：80%（`22937`）触发、60%（`17203`）为目标。
阈值仍进入 ContextPlan 和 `run.started` metadata，但 soft 决策只接受与当前
profile/renderer/ToolSet/policy 同 scope 的 `SoftContextEstimate`；边界语义是 `>= trigger`。
每个新 Turn 重新投影所有 completed bundles，从最旧端移动完整 bundle，不能切开 Tool call/
result 身份；每层投影后重算 soft estimate 并独立执行 `AdmissionUpperBound` 硬准入。
projection 绑定来源 history、collapsed/preserved Turn/digest 与 renderer/policy，相同 canonical
输入产生字节一致模型视图。SQLite 完整 transcript 不变，换大窗口可重新纳入旧 Turn。旧
pair-only/tail boundary 仅兼容解码；受保护 sections、continuation/Tool headroom、最近完整
Turn 与当前 user 仍超硬预算时 fail closed。

`utf8-bytes-upper-bound-v1` 是硬 admission fallback：没有模型 tokenizer 时每个 UTF-8 byte
按一个 token 计安全上界，并计入实际 renderer、Provider Tool schema、模板、Tool growth、
continuation 与 output reserve；它从不显示为真实 token。Provider terminal usage 只对 exact
request digest 权威。服务端用匹配 scope 的完整 observation 对 byte estimate 建立有界比例和
误差，生成 soft estimate 与 next-turn preview；样本不足、比例异常或 scope 漂移时明确
unavailable，不退回混用 byte/Provider 数字。完整合同见
[Context count domains](context-count-domains.md)。
...
- timeout 是模型目录的一部分，而不是请求参数。默认画像为首帧 60 秒、帧间空闲 20 秒、
  单次调用 120 秒；Broker 以 `model_first_frame_timeout`、
  `model_stream_idle_timeout`、`model_turn_deadline` 和 `model_transport_timeout` 区分阶段。
  HTTP read 不再用 10 秒通用 timeout 提前覆盖逻辑 deadline。
- 每轮最多接受 1 MiB/4096 个 provider 原始帧，并合并为最多 128 个 content IPC frame；
  Broker error 的 code/retryability 由 Control Plane 绑定到 canonical failure。
- 普通无 Tool content 每累计 64 个 accepted UTF-8 bytes，在最多 2048 个尾部 code point 中
  检查精确 suffix cycle；unit 为 32..512 UTF-8 bytes、至少 3 份且总证据至少 512 bytes。
  命中只截尚未发送的重复尾部并立即关闭 Provider stream。用户明确请求逐字重复较长内容也
  可能触发，这是防止无界资源消耗的保守取舍；不把 detector 描述为一般语义质量判断。
- 每个逻辑模型 iteration 的一次 HTTP 调用在首帧前 timeout 且零 Provider 输出时最多透明
  重试一次；收到任何帧后不重试。canonical provider attempt 仍最多 2 个，仅首个、零
  assistant/tool 输出的精确 context/media overflow 可进入 recovery attempt 1。恢复
  projection 已在原 profile 下重新 admission；第二次 overflow、取消、idle/deadline 或任何
  部分输出均直接收敛。
- Agent 指令采用 direct-answer-first Tool policy：只有外部/workspace 状态或用户明确要求的
  操作才使用 Tool，创作与自包含问答直接生成；算术表达式按常规运算优先级并在回答前复核。
  其余 ToolSet 与执行权限保持不变；无业务 Echo 在 policy 层移除，Tool 阶段结束再由末尾
  finalization marker 做结构化相位切换，避免仅靠提示词约束小模型。任何残余第三次 Tool
  请求以 `model_tool_loop` 精确分类且绝不执行。
