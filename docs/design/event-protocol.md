...
| `model.request.started` | durable | request_id、iteration、attempt、recovery_id、provider_call_index、context plan ID/digest、request digest/bytes、runtime estimated input、message/tool counts、Tool result call IDs | exact request 已完成 admission、准备释放给 provider；每个逻辑调用或 overflow recovery request 一次，只由控制面发出 |
| `model.transport.attempt` | durable diagnostic | version、request_id、iteration、provider_call_index、attempt/max_attempts、phase、outcome、elapsed_ms、first_frame_ms | 嵌套在开放 request 内；每个实际首帧 HTTP attempt 固定一个 started/finished pair，不含 endpoint、prompt、body、frame 或错误原文 |
| `model.response.finished` | durable | request_id、iteration、attempt、recovery_id、provider_call_index、outcome、单调用 input/output usage、usage_complete、error_code | 与 request 一一配对；outcome 为 tool_use/end_turn/repetition_truncated/error/cancelled，只由控制面发出 |
...
| `tool.call.requested` | durable | call_id、tool_id、arguments | 当前最多顺序调用两次有效 Tool；新 Run 的 stat/read/glob/grep/edit/write/exec 与旧 Run 的 Echo 均按各自冻结 ToolSpec 校验，call ID 唯一 |
...
| `run.completed` | durable | reason、model_iterations、usage | 唯一成功终态；reason 只能是 `end_turn|max_output|repetition_truncated` |
...
receipt 自身和 projection metadata 再绑定一个 projection digest。旧 Run / 原型中的 Echo
canonical request/result 各最多 8192 bytes；文件结果最多 12288 bytes且 provider projection
最多 8192 bytes；旧 Echo 单结果 provider projection 最多 4096 bytes，单 Run
...
仍由 replay allowlist 验证；新 Run 使用不含 Echo、包含 stat/read/glob/grep/edit/write/exec
及已启用可选能力的 runtime digest。
...
`run.completed` 的同一终态事务还按
[Completed Turn context contract](completed-turn-context.md) 晋升唯一
`completed-turn-context-v1` bundle。其 item 顺序是 user、零至两个 assistant Tool call/低权限
Tool result receipt、final assistant，并与 call/Tool/arguments/result digest 对账。event journal
仍是发生事实，bundle 是后续模型历史；二者不能互相重建。failed/cancelled/interrupted、部分
正文或未收敛 Tool 不晋升，旧 pair-only Turn 只按低保真 v0 decoder 读取。
...
`qwen3.5:2b` 的对应容量值为 `262144 / 32768 / 28672 / 22937 / 17203 / 4096`；这些
来自受信模型资格与策略，不是 Worker 声明。`completed-turn-collapse-v2` 只表示把最旧
完整 completed Turn bundles 投影成绑定 source/collapsed/preserved digest 的 content-free
marker，并至少保留最近一个完整 bundle；它不是模型 summary 或语义摘要，SQLite 中的完整
transcript 不因投影而删除。旧 pair/tail snapshot 仍可版本化解码，但不能静默套用
`ordered-sections-v7` / `prompt-section-registry-v6`。
...
仍暴露冻结的 ToolSet，达到预算后精确收窄为空。最终请求会在最后一个 Tool result 之后
追加固定、受信且不拼接结果内容的 system finalization marker；它参与完整 token/byte
admission 和 request digest，但不另建 journal 事件或 Tool capability。模型仍请求 Tool 时
Broker 使用 `model_tool_loop`，且绝不执行第三个 Tool。旧 feature-only Run 仍按 one-shot
规则验证；旧 Echo-bearing ToolSet digest 继续可回放，新 release ToolSet 不含 Echo。

所有 usage/capacity 字段的语义服从
[Context count domains](context-count-domains.md)。旧 `estimated_input_tokens` 只解码为
`AdmissionUpperBound` 兼容值，不是 Provider token；Provider actual 只对同一
`request_digest` 的 complete response 权威。事件/metadata 必须携带 basis/version/availability，
不能由客户端把不同 domain 的裸整数相减。

`model.transport.attempt` 不创建第二套模型调用边界。这里的 `attempt=1..2` 只表示同一个
exact request 在取得首帧前的 HTTP 传输尝试，与 overflow recovery 的 request-level
`attempt=0|1` 不同。started 固定 `elapsed_ms=0/outcome=null`；finished 只允许固定 outcome，
首帧成功时 `first_frame_ms=elapsed_ms`，其它结果不得带 TTFB。每个逻辑调用固定有一个
request/response boundary pair，每个实际 HTTP attempt 固定再有一个 transport diagnostic
pair；实际尝试数为 `N` 时，durable 事件总数因此是 `2 + 2*N`。严格 replay 要求 attempt 从
1 连续、max 不漂移、start/finish 成对且位于匹配 request 内；Gateway
重启会先为开放诊断写合成 finish，再闭合开放 response 和 terminal。每秒等待数字只在浏览器
内存更新，不写 Event、SQLite 或日志；Provider frame、token delta 和 timer tick 也不形成
durable event。

Control Plane 的 request observer 仍是权威边界：它失败时不得打开 Provider HTTP。transport
observer 只是内容安全的诊断，Broker 不因 UI/诊断回调异常重复 HTTP 或改变已取得的模型流；
其 canonical writer 和 replay 仍对已提交的任何诊断 fail closed。
...
当前 release Ollama transport 固定使用流式 `POST /api/chat`，不调用 `/api/generate`。
普通文本超过 12 KiB assistant commit ceiling，或 Provider 以 `done_reason=length` 结束且已经
产生可用正文时，Broker 只接纳 UTF-8 完整边界内的前缀、追加恰好一次受信截断 marker，并
继续有界消费后续 NDJSON，直到取得并校验唯一终帧及最终 usage。该调用仍以
`model.response.finished.outcome=end_turn` 闭合且 `usage_complete=true`，正文只由一个
`assistant.block.finished` 提交，Run 用 `run.completed.reason=max_output` 区分有界截断完成。
达到上限时若存在 Tool call 或没有可见正文，或流缺失/重复终帧、usage/字段/帧协议非法，
则仍以相应受信错误 fail closed；不得合成 `max_output` 成功，也不得提交 partial Tool 结果。

普通无 Tool response 的 bounded exact-suffix guard 命中时是另一种受限成功组合：Broker
保留已发正文、删除尚未发出的重复尾部、追加一次固定 marker，并立即关闭 HTTP response。
因为没有等待 Ollama 终帧，该 response 必须精确为
`outcome=repetition_truncated,input_tokens=0,output_tokens=0,usage_complete=false,error_code=null`；
随后必须有一个非空 `assistant.block.finished`，唯一终态必须是
`run.completed.reason=repetition_truncated` 且 aggregate `usage.complete=false`。该组合只允许
普通文本、零 request Tool schema/Tool call；自然 terminal、无正文或先被观察到的显式取消
继续走原合同。零 usage 字段不表示实际零计算量，不进入 soft calibration。marker 随 completed
Turn bundle 进入下一轮历史；下一轮 soft preview 可因 incomplete observation 显示
unavailable，但 exact rendered request 仍独立执行硬 admission。
...
- `GET /api/sessions/<conversation-id>?limit=1..64&before=<opaque-cursor>` 默认返回最新
  32 个 Turn；下一页只能使用 `turn-page-v2.page.next_before_cursor`。cursor 经 HMAC 认证并
  绑定 Agent、Conversation、revision 与服务端 Turn position 边界，不能由客户端构造、
  跨会话复用或在 rename/Gateway 重启后继续使用；无效或过期 cursor 返回有界
  `invalid_session_cursor`；
- `PATCH|DELETE /api/sessions/<conversation-id>` 重命名或删除 Conversation；
- `POST /api/sessions/<conversation-id>/continue` 以 `mode=continue|branch` 从 completed
  head 新建独立 Conversation，源保持不变；
- `POST /api/sessions/<conversation-id>/runs` 新建一个 Turn 和其唯一 Run；
- `GET /api/sessions/<conversation-id>/preparation` 返回 prompt-free 瞬态准备阶段；idle 的
  `operation_id=null`，preparing/cancelling 的非空 ID 在阶段变化及 preparation→active Run
  handoff 中保持稳定；
- `POST .../preparation/cancel` 的 JSON 只能携带上述 exact `operation_id`，以同一个
  cancellation signal 取消摘要/压缩准备或 handoff 后的 Run；`202` 只表示请求已受理，调用方
  继续 GET 到 idle 才能确认收敛。旧 ID 返回 `stale` 且不得触碰后来操作，避免 ABA；
...
`completed` context bundle。`run.failed`、`run.cancelled` 或 Gateway 重启留下的
`interrupted` Turn 没有 assistant history；其 partial block、Tool result 和 ephemeral
delta 不进入后续 ContextPlan。Session detail 仍为这些 Turn 返回
`turn-terminal-v1 {code,stage,retryable,duration_ms}`，但不返回 Provider/Worker message；
该 UI 恢复事实与可晋升模型历史严格分离。
...
- 右侧 Conversation 面按 `turn_id` 展示 session API 返回的 durable user message、状态和
  ...
  active Run 的 Turn 上，终态后由 durable transcript 对账。同一 Run 的 canonical assistant
  已出现时，terminal refresh 必须禁止再次附加 live assistant buffer；`turn_id`/`run_id`
  客户端身份差异不能制造第二张 Turn 卡；
...
`context-projection-v2` 是独立于 EventEnvelope 与 `run-ui-v2` 的模型视图恢复边界；旧 codec
仍可严格解码，但不能静默按 context schema v6、`ordered-sections-v7` /
`prompt-section-registry-v6` 复用。
...
boundary 固定绑定 Agent/generation、Conversation/Turn/Run 和 source revision、admission 与
active ContextPlan ID/digest、完整 history/bundle digest、model profile、compression policy、
ToolSet/catalog/policy、renderer/section registry、window strategy 和计数域 metadata。普通
projection 不保存 section、user/assistant、Tool result 或 Provider request 正文。
`semantic-summary-v2` 额外引用一个有界 Conversation-scoped projection：五类派生摘要、source
Turn/digest、parent snapshot、model/profile、真实 prompt/policy/renderer、Provider request
digest 与独立 usage；source 和摘要在主模型中仍是低权限 data。v1 snapshot 只可旧 codec
解码，永不生成或提升为 v2。

单条 boundary canonical JSON 最大 16 KiB；每 Run 只有一条 current boundary，
`admission|replay|manual_compact|semantic_summary|overflow_recovery` 使用同一 codec。替换必须
提供旧 digest 做 SQLite CAS。overflow recovery 的 transition event、journal state 和 active
plan metadata 同事务提交；随后所有 Tool-loop Provider request、usage、inspection 与 replay
只引用 active recovery plan。崩溃因而只能留下完整旧态或可解释、可验证的新态，不能读到
半写或混合身份。
...
run.started [model-call-boundaries-v1 + sequential-multi-tool-v1 + one-shot-overflow-recovery-v1]
  ...
  ├─ model.request.started #3 [both Tool result IDs; finalization marker; empty ToolSet]
...
- 每个逻辑 provider 调用增加一个 request 和一个 response durable boundary；每个实际 HTTP
  attempt 另增加一个 transport started 和一个 transport finished durable diagnostic。实际
  attempt 数为 `N` 时总数为 `2 + 2*N`，不按 provider frame、token 或 timer tick 写入。每 Run
  最多 512 live events、1 MiB live bytes、256 KiB durable bytes；
  普通事件为最坏
...
- 每轮最多 1 MiB/4096 个 provider 原始帧，Control/Broker 合并为最多 128 个 content IPC
  frame。
