...
`["model-call-boundaries-v1"]`，随后必须遵守下面的 request/response 配对；没有该字段的
...
| `model.request.started` | durable | request_id、iteration、context plan ID/digest、request digest/bytes、runtime estimated input、message/tool counts、Tool result call IDs | exact request 已完成 admission、准备释放给 provider；每 iteration 一次，只由控制面发出 |
| `model.response.finished` | durable | request_id、iteration、outcome、单调用 input/output usage、usage_complete、error_code | 与 request 一一配对；outcome 为 tool_use/end_turn/error/cancelled，只由控制面发出 |
...
当次 EffectiveToolSet 的保守估算，因此 Tool result 回流后的第二次调用不再复用 base
ContextPlan 的初始估算。`request_bytes`、`message_count`、`tool_count` 和
`tool_result_call_ids` 均有严格上限；首轮结果 ID 为空并暴露 ContextPlan ToolSet，后续轮
必须引用已经 canonical finished 的 Tool result 且 ToolSet 收窄为空。
...
- `exact`：该 immutable ContextPlan 仍在当前 Gateway 的有界 Run retention 中。Control
  ...
  `id/role/trust/provenance/cache/truncation/estimated_tokens/content_bytes/content_digest`。
...
## 持久 operation 与 provider ledger

`operation_ledger` 为未来有外部副作用的 capability broker 提供持久底座：
...
- 当前唯一 `builtin/echo` 是进程内只读 Tool，没有特权副作用 executor 使用该
  ledger。它是之后 capability broker 的恢复契约，不是当前已支持“恰好一次”
  Shell/File/Skill/MCP 的声明。
...
```text
run.started [model-call-boundaries-v1]
  ...
  ├─ model.request.started #2 [finished Tool result IDs; empty ToolSet]
  │    └─ model.response.finished #2 [end_turn | error | cancelled]
...
3. 声明 model boundary feature 的 Run，iteration 必须从 1 连续递增，每次 request 有且
   只有一个匹配 response；request 只能在上次 response 和必要 Tool result 完成后出现。
