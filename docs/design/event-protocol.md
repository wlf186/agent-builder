...
`["model-call-boundaries-v1","sequential-multi-tool-v1"]`，随后必须遵守下面的
...
| `tool.call.requested` | durable | call_id、tool_id、arguments | 当前最多顺序调用两次 `builtin/echo`，每次参数上限 8192 UTF-8 bytes，call ID 唯一 |
...
```text
run.started [model-call-boundaries-v1 + sequential-multi-tool-v1]
