...
```text
[Harness：检测到响应进入重复死循环，已截断重复尾部；后续对话应仅参考此前保留的内容。]
```

marker 最多出现一次并计入 12 KiB assistant commit ceiling。检测阈值内、已经实时发出的少量
...
保持既有 schema，并明确 `complete=false`。Web 把该 terminal 显示为“检测到重复循环，回答已
截断并提交”，而不是错误、取消或完整 Provider usage。
