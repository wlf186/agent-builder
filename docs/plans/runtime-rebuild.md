...
- 共享不可变 `ToolSpec` 同时生成 provider schema、Worker/Control Plane 校验和
  toolset digest；
- 当前模型画像为 native `262144`、operational `32768`、output reserve `4096`、硬输入
  `28672`，并已生成 80% trigger / 60% target 的动态压缩策略；
...
- runtime release ToolSet 包含 descriptor-anchored stat/read/glob/grep，以及要求
  ...
  预算耗尽后 provider ToolSet 收窄为空并追加受信 finalization marker；Echo 只留作旧回放/
  原型测试，不进入新 Run policy；
...
- Web 中央按 durable Turn 展示多轮 user/assistant transcript，左侧导航 Agent/Conversation，
  默认关闭的右侧检查器把所选 Run 按 User/Harness/LLM/Tool 四泳道与 canonical `seq`
...
PID 和 Run root residual 均为零。最终全量为 `776 passed`，治理、diff、shell/JavaScript
语法和 `AGENTS.md -> CLAUDE.md` 门禁通过。原问题 Conversation 全程只读，资格使用新建的
隔离 Conversation；有界 aggregate 证据位于被忽略的
`.runtime/test-results/RR-REPETITION-20260724-01/summary.json`。实现快进合并到 `main` 后，
合并后的 checkout 再次完成 `776 passed`、governance 和固定模型 `/health` 检查；临时
worktree 与 feature branch 已清理。
...
- [x] 持久 Agent registry 和 generation/version 状态机支持 create/list/get/rename/upgrade/delete。
...
- [x] same-generation rename 不 drain 或重建环境，并在 manifest/registry commit 崩溃点幂等
  收敛；create/upgrade/delete 在每个 staging、rename、registry commit 崩溃点同样可恢复。
...
惰性 runtime、共享有界 Broker、Agent-scoped session/Run/context API、rename/drain/upgrade/delete；
...
状态：`done`（2026-07-24 完成 B+ 实现、故障矩阵、真实模型和生命周期资格，并合并到
`main`）。
...
- [x] 完成 `REP-R03-07..08`：三次目标五轮真实模型、旧采样 forced guard、正常/强制 stop、
  residual audit 和最终 `776 passed`；
- [x] 在合并后的 `main` checkout 复跑 `776 passed`、governance、`/health`，并清理临时
...
2026-07-21 的 Web 实机发送复验补上 optional ToolSet 组合负面边界：research、Skill、
extension 可独立存在，replay 现在从固定 runtime catalog 枚举并核对其确定性子集，而不是
维护容易漏项的 digest 列表。开启 `document/extract_text`、未开启 Skill/extension 的真实
系统 Agent 已能将模型回答原子收敛为 `run.completed`；catalog 外 tool、digest/visible ID
漂移仍 fail closed。

### Follow-on record — Conversation-first Web workspace

这不是冻结路线的第 29 项，也不改变上方 28 项的完成口径。2026-07-21 对交互壳层做
conversation-first 重排：左侧只导航 Agent/Conversation，中央保留 durable Turn transcript
和固定 composer，原四泳道 replay、完整事件消息体、ContextPlan 检查与子 Run 定位收进
默认关闭的右侧 inspector。点击 Turn 会明确打开对应 Run；窄屏时 navigation/inspector
互斥，Esc 和 backdrop 可关闭。Composer 支持 Enter 发送、Shift+Enter 换行、快捷任务提示，
模型选择保持显式，上下文压缩和 Slash Command help 进入按需更多选项。
Agent-scoped Run admission 与 composer 使用同一 `{message,model_id,compact}` 合同；状态与
错误同时显示在顶部和 composer 下方，避免请求被拒绝时呈现为“没有反应”。
消息区新增有条件 follow-latest：新 Turn 和流式 assistant block 自动吸底，用户向上滚动后
暂停并显示恢复按钮。Composer 新增会话上下文条，从最新 parent Run 的受验证
`run.started` 读取输入估算、运行窗口、压缩触发点和硬输入预算；选择旧 Run 或子 Run 不得
覆盖当前会话指标，尚未执行的会话只显示“尚未计算 / 当前模型窗口”。

验收证据：全量 `565 passed`，JavaScript/治理/diff 门禁通过；20815 真实
`qwen3.5:2b` Run 以 `run.completed` 返回 `CONTEXT-OK`，上下文条同步显示
`14,659 / 32,768 tokens · 45%` 且 progressbar now/max 为 `14659/32768`。浏览器位置断言
覆盖自动吸底 `bottom=0`、用户回看 `top=0/follow=false` 和点击恢复
`bottom=0/follow=true`；验收 Conversation 已删除，无测试会话残留。

设计只吸收公开产品的分层原则，不复制其品牌资产或专有布局：Codex 的 project/thread 与
按需 review、Kimi/豆包的低摩擦 prompt 入口、OpenClaw/Hermes 的 conversation 与高级控制
分层共同支持“默认完成任务、按需查看运行”的取舍。浏览器仍只是 transient projection；
隐藏 inspector 不暂停 SSE、不删减 canonical event，也不建立第二份状态机。权威行为见
[architecture](../design/architecture.md)，用户入口见 [README](../../README.md)。

### Follow-on record — Model timeout and long-response qualification

2026-07-21 对真实长响应失败做了分阶段治理：ModelCatalog 现在绑定 queue/first-frame/
stream-idle/turn timeout，默认 qwen 为 30/60/20/120 秒，Run wall deadline 为 240 秒；只有
首帧前零输出 timeout 最多透明重试一次。阶段错误进入 canonical terminal 与 Web 检查器。
默认输出预留增至 4096，32K 运行窗口的硬输入预算相应为 28672；visible output、帧数、
stream bytes 和 wall time 仍保持独立硬上限。受信 Agent 指令同时采用 direct-answer-first
Tool policy，减少创作/自包含问答进入 Provider buffered Tool call 的概率。

真实 `iollama:11434/qwen3.5:2b` 资格回归以约 500 词英文诗请求验证：旧实现会在 10.003 秒
以通用 transport timeout 失败，修改后临时 Run 在 13.891 秒收敛为 `run.completed`，Provider
累计 usage 为 input 3406 / output 516。临时 Conversation 已删除；验证没有把 prompt、响应
正文或访问令牌写入计划文档。

2026-07-21 修正多 Tool 调用后的下轮上下文指标：本轮 Provider 实际用量继续累计所有完整
调用，表示计算/成本；下轮基线改为第一次完整 ToolSet 请求的实际 input 加最终回答调用的
实际 output。中间 Tool call/result、重复 prompt 和最终收窄为空的 ToolSet 不进入持久会话
基线。UI 展开显示两个组成数；缺少逐调用边界的 snapshot-only 多调用 Run 停止推导，不再
用最后一次 input 加累计 output 产生看似精确的低估值。
复现样本 Turn4 的 durable 边界验证为三次调用累计 `4717 in + 314 out = 5031`；新基线取
首次 `1863 in` 与最终 `186 out`，显示约
`2049 / 32768`。全量 `579 passed`、治理和 diff 门禁通过，20815 已加载新投影脚本。

### Follow-on record — Separate rename from generation upgrade

2026-07-21 将普通 Agent 的显示名修改与运行代升级拆开。`PATCH /api/agents/<id>` 现在使用
可恢复的 `renaming → active` 状态机，同 generation 原子更新 registry/manifest；名称未变化
时零写入，正常 rename 不 drain runtime、不新建 venv，也不触碰 Conversation、workspace、
Skill 或持久依赖。manifest publication 前后和 active commit 前后的故障均由启动恢复幂等
收敛。Web 的普通入口改为“重命名”，generation 显示弱化为运行环境版本；原 upgrade 收进
“高级操作”并更名为“重建运行环境”，说明它只适用于基础环境、安全策略或 Agent 定义发生
实质变化的场景，并明确会排空运行时。系统 Agent 拒绝 rename/upgrade/delete。

验收证据：全量 `579 passed`，治理与 `git diff --check` 通过；20815 实际创建临时普通 Agent
后执行 rename 返回 200，名称改变而 generation 与 Worker interpreter inode 均保持不变，
未创建 `generations/`。随后删除返回 204，data/runtime 两个 Capsule root 均不存在。

### Follow-on record — Eliminate the Echo-induced Tool loop

2026-07-21 对会话 `0d839af6f7dd41b89b62b96412f6a28d` 的 Turn 4 做了 exact-request
取证。Run `0a81c6995e30419f91b2d9411f85ef8f` 在 `4.169s` 内完成三次 Provider
调用：前两次分别以 `1546/34`、`1602/36` input/output usage 错误选择
`builtin/echo`；第二次 Tool result 后预算已经耗尽，第三次 request 的 `tool_count=0`，但
模型仍生成第三个 `builtin_echo` call，于是旧 Broker 以通用 `model_protocol_error` 终止。
当时 ContextPlan 估算 `15264`、硬输入预算 `28672`、运行窗口 `32768`，因此可排除上下文
溢出、排队和 timeout。当前保留的两个 failed Turn 均为同一“两次 Echo → 空 ToolSet 后仍
Echo”形状，没有第二类高频失败证据。

修复把 catalog 与 release policy 明确拆开：Echo contract 继续用于旧 journal replay 和
原型单测，但不再进入任何新 release Run 的 EffectiveToolSet。第二个 Tool result 回流时，
Broker 仍把 provider ToolSet 收窄为空，同时在该不可信结果之后追加固定、不可插值的受信
system finalization marker；marker 参与逐请求 Token/byte admission 和 request digest，要求
模型以普通 assistant text 完成本轮。对原失败第三请求只加入该 marker 的真实 qwen 对照在
`8.144s` 内返回约 `266` 个英文词、零 Tool call；只加 JSON schema 仍重复 Echo，故未采用。
任何残余第三次 Tool 请求现在精确归类为 `model_tool_loop` 且绝不执行。

验收证据：新增“release policy 无 Echo”“两次 Tool 后 marker + 空 ToolSet + 可见回答”和
“残余第三次 Tool 使用专用错误码”回归；历史 Echo toolset 的 current/optional digest 组合
仍可回放和启动恢复。全量 `581 passed`，治理与 `git diff --check` 通过。受管重启后 20815
真实发送相同 `write a poem for around 300 words`，一次 Provider 调用即
`run.completed`；`run.started.visible_tools` 为 9 个当前真实能力且不含 Echo，临时
Conversation 已删除。
