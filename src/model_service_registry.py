...
from .models import ModelProvider, ModelServiceConfig
...
class ModelServiceRegistry:
    ...
    @staticmethod
    def _copy_services(
        services: Mapping[str, ModelServiceConfig],
    ) -> Dict[str, ModelServiceConfig]:
        return {
    ...
    def _secret_values(
        ...
        return {
            self._secret_key(name): config.api_key
            for name, config in services.items()
        }
    ...
    def _commit_services(
        self,
        previous: Mapping[str, ModelServiceConfig],
        candidate: Mapping[str, ModelServiceConfig],
    ) -> None:
    ...
    def _load_services(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                parsed: Dict[str, ModelServiceConfig] = {}
                for name, raw_config in raw_configs.items():
                    secret_key = self._secret_key(name)
                    has_legacy_field = "api_key" in raw_config
                    plaintext = raw_config.pop("api_key", None)
                    raw_config["api_key"] = (
                        plaintext
                        if has_legacy_field and plaintext is not None
                        else copy.deepcopy(encrypted.get(secret_key))
                    )
                    config = ModelServiceConfig(**raw_config)
                    if config.name != name:
                ...
                self.services = {}
                print(f"加载模型服务配置失败: {type(exc).__name__}")

    def _save_services(
        self,
        previous: Optional[Mapping[str, ModelServiceConfig]] = None,
        candidate: Optional[Mapping[str, ModelServiceConfig]] = None,
    ) -> None:
